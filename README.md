...
Supported scenarios: `profile`, `storage-save`, `storage-get`, `room-sync-msg`, `mixed`

### Low-latency room sync
Room sync now uses the client message channel `Room/ClientSync` instead of the removed `Room/Sync` API:
...
- The load test scenario is `room-sync-msg`.
